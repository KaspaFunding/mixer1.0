...
function createOrUpdateBatchFile(batPath, isPublic, kaspadExePath) {
  const actualBatPath = getWritableBatPath(batPath);
  const batContent = generateBatContent(isPublic, kaspadExePath);
  fs.writeFileSync(actualBatPath, batContent, 'utf8');
  console.log(`[Node Starter] Created batch file at: ${actualBatPath}`);
  console.log(`[Node Starter] Kaspad path in batch: ${kaspadExePath || 'dynamic'}`);
...
function startNodeProcess(exePath, batPath, isPublic) {
  ...
  console.log(`[Node Starter] exePath: ${exePath}, exists: ${exeExists}`);
  console.log(`[Node Starter] batPath: ${batPath}, exists: ${batExists}`);
  
  if (!exeExists && !batExists) {
    return { started: false, found: false };
  }
  
  // Always prefer creating/updating batch file with the correct kaspad path
  if (exeExists) {
    try {
      const actualBatPath = createOrUpdateBatchFile(batPath, isPublic, exePath);
      console.log(`  ${batExists ? 'Using' : 'Creating'} start-kaspad.bat (${isPublic ? 'Public' : 'Private'} mode, auto-restart enabled)`);
      console.log(`  Batch file: ${actualBatPath}`);
      console.log(`  Kaspad exe: ${exePath}`);
  ...
  if (batExists) {
    // Batch file exists but exe doesn't - try using the batch file anyway
    const actualBatPath = createOrUpdateBatchFile(batPath, isPublic, null);
    console.log(`  Using existing start-kaspad.bat (${isPublic ? 'Public' : 'Private'} mode, auto-restart enabled)`);
    startNodeWithBatch(actualBatPath, isPublic);
    return { started: true, usedBatch: true };
  }
...
// If kaspadExePath is provided, use it directly; otherwise use dynamic resolution
function generateBatContent(isPublic = false, kaspadExePath = null) {
  ...
  // If we have an explicit path to kaspad.exe, use it directly
  if (kaspadExePath && fs.existsSync(kaspadExePath)) {
    const kaspadDir = path.dirname(kaspadExePath);
    // Escape backslashes for batch file
    const escapedExePath = kaspadExePath.replace(/\\/g, '\\\\');
    const escapedDirPath = kaspadDir.replace(/\\/g, '\\\\');
    
    return `@echo off
REM Kaspa Node Auto-Start Script (Auto-generated with explicit path)
REM This script starts kaspad.exe and automatically restarts it if it crashes

title Kaspa Node - Auto-Restart Script

REM Use the explicitly provided kaspad.exe path
set "KASPAD_EXE=${escapedExePath}"
set "KASPAD_DIR=${escapedDirPath}"

:start_node
echo Starting Kaspa Node (kaspad.exe)...
echo Directory: %KASPAD_DIR%
echo Executable: %KASPAD_EXE%
...
if errorlevel 2 goto start_node
echo Stopping by user request.
goto :eof
`;
  }
  
  // Otherwise, use dynamic path resolution for maximum compatibility
