...
async function fetchNodeData(rpc) {
...
// Extract peer count from peer data
function extractPeerCount(peers) {
  let peerCount = 0;
  let peerDetails = [];
  ...
  return { peerCount, peerDetails };
}

// Calculate mempool statistics
function calculateMempoolStats(mempoolEntries, info) {
  const mempoolStats = {
    transactionCount: 0,
    totalSize: 0,
    oldestTxAge: null,
  ...
  if (mempoolEntries && mempoolEntries.entries) {
    ...
    entries.forEach(entry => {
      if (entry) {
        ...
        // Use transaction size from mass or size field
...
// Extract fee rates from fee estimate
function extractFeeRates(feeEstimate, mempoolStats) {
...
function calculateSyncProgress(info, dagInfo) {
...
// Display node status to console
function displayNodeStatus(info, dagInfo, serverInfo, peerCount, mempoolStats, syncProgress) {
  console.log('\n📊 Node Status:');
...
// Format status response object
function formatStatusResponse(info, dagInfo, serverInfo, peerCount, mempoolStats, feeEstimate, syncProgress, isHealthy) {
...
async function checkNodeStatus() {
  try {
    ...
    // Get node information in parallel
    const [info, dagInfo, peers, feeEstimate, serverInfo, mempoolEntries] = await fetchNodeData(rpc);
    ...
    // Extract peer count
    const { peerCount } = extractPeerCount(peers);
    
    // Calculate mempool statistics
    let mempoolStats = calculateMempoolStats(mempoolEntries, info);
    mempoolStats = extractFeeRates(feeEstimate, mempoolStats);
    
    // Calculate sync progress
    const syncProgress = calculateSyncProgress(info, dagInfo);
    
    // Display status and get health status
    const isHealthy = displayNodeStatus(info, dagInfo, serverInfo, peerCount, mempoolStats, syncProgress);
    
    // Format and return response
    return formatStatusResponse(info, dagInfo, serverInfo, peerCount, mempoolStats, feeEstimate, syncProgress, isHealthy);
  } catch (err) {
    console.log('\n✗ Node Status: Error checking node');
    const errorMsg = err.message || String(err);
    // Check if error is related to BigInt conversion
